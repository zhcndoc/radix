	...
	data={[
		{
			...
			description: (
				<>
					将默认渲染元素更改为作为子元素传递的元素，并合并它们的属性和行为。
					<br />
					<br />
					请阅读我们的 <a href="../guides/composition">组合</a> 指南以获取更多详情。
				</>
			),
		},
		{
		...
		},
		{
			...
			description: (
				<span>
					滑块的阅读方向。如果省略，则从 <Code>DirectionProvider</Code> 全局继承，或者假定为 LTR（从左到右）阅读模式。
				</span>
			),
		},
		{
...
默认情况下，`Slider.Thumb` 会渲染一个在视觉上隐藏的 `input`，用于提交表单。若要重新组合、移动或排除该 `input`，可以改用更底层的部件构建每个滑块。

> **重要：** 这些部件不稳定，并带有 `unstable_` 前缀，因此其 API 可能会在未来版本中发生变化。

- `Slider.unstable_ThumbProvider` 提供滑块状态，并接受一个可选的 `name`，用于提交值。
- `Slider.unstable_ThumbTrigger` 是可拖动的滑块元素。
- `Slider.unstable_BubbleInput` 是 `Slider.Thumb` 默认渲染的视觉隐藏 `input`。不需要提交表单时可以省略它。
