...
<CssVariablesTable
	data={[
		{
			cssVariable: "--radix-navigation-menu-indicator-translate-x",
			description:
				"指示器的水平偏移量，根据活动触发器的位置计算得出。菜单为水平时存在。",
		},
		{
			cssVariable: "--radix-navigation-menu-indicator-translate-y",
			description:
				"指示器的垂直偏移量，根据活动触发器的位置计算得出。菜单为垂直时存在。",
		},
	]}
/>
	...
	data={[
		{
			...
			description:
				"视口在可见／隐藏状态下的宽度，根据活动内容计算得出",
		},
		{
			cssVariable: "--radix-navigation-menu-viewport-height",
			description:
				"视口在可见／隐藏状态下的高度，根据活动内容计算得出",
		},
	]}
/>
...
<NavigationMenu.Root>
	...
	{/* NavigationalMenu.Content 激活时将在这里渲染 */}
	<NavigationMenu.Viewport />
</NavigationMenu.Root>
```
